...
{
    class ConvertXmlsToXlsxFileForEmail
    {
        #region PROPERTIES
        ...
        #endregion
        #region CONSTRUCTOR
        public ConvertXmlsToXlsxFileForEmail(string src, string dest)
        {
            // assign properties
            Source = src;
            Destination = dest;
            ...
            foreach (var _f in _files)
            {
                ...
                foreach (var atty in query_attorneys)
                {
                    ...
                    if (String.IsNullOrWhiteSpace(atty.Attribute("email").Value))
                    {
                        reasonElim.NoEmail++; continue;
                    }
                    ...
                    var dr_shortTitleEmail = dr_shortTitle;


                    string dr_shortTitleEmail50Chars = "";
                    if (dr_shortTitleEmail.Length >= 50)
                    {
                        dr_shortTitleEmail50Chars = dr_shortTitleEmail.Substring(0, 50);
                        Console.WriteLine(dr_shortTitleEmail50Chars);
                    }
                    ...
                    //string dr_shortTitle50Chars = "";
                    //// test shortTitle for 50 character limit
                    //if (dr_shortTitle.Length >= 50)
                    //{
                    //    dr_shortTitle50Chars = dr_shortTitle.Substring(0, 50);
                    //    Console.WriteLine(dr_shortTitle50Chars);
                    //}
                    ...
                    dr["shortTitleEmail"] = dr_shortTitleEmail;
            ...
            {
                var conversionInfoFilename = System.IO.Path.Combine(Source, "conversion_info_email.txt");
        ...
        private DataTable InitiateCircuitCourtDataTable()
        {
            ...
            dt.Columns.Add(new DataColumn("shortTitleEmail", typeof(string)));
